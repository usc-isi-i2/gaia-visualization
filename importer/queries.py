...
def delete_ori_cluster(graph_uri):
    open_clause = close_clause = ''
    if graph_uri:
        open_clause = 'GRAPH <%s> {' % graph_uri
        close_clause = '}'
    return '''
...
def delete_ori_clusterMember(graph_uri):
    if graph_uri:
        open_clause = 'GRAPH <%s> {' % graph_uri
        close_clause = '}'
    return '''
...
DELETE { 
    ...
    ?type_assertion a rdf:Statement ;
        ...
        rdf:object ?t
    ?prototype aida:justifiedBy ?j .
    ?type_assertion aida:justifiedBy ?j
} 
WHERE {
    ?prototype aida:hasName ?name .
    ?type_assertion a rdf:Statement ;
